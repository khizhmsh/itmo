        ...
        if (input.contains("login") || input.contains("reg") || isLogin) {
            if (input.contains("login") || input.contains("reg")){
                if (parts.length == 3) {
                    ...
                    request.setNameCommand(parts[0]);
                    request.setLogin(parts[1]);
            ...
            if (parts.length == 1) {
                ...
                if (input.contains("remove_greater") && !input.contains("remove_greater_key"))
                    request.setRoute(RouteGenerator.createRoute(0L));
                return request;
            } else if (parts.length == 2) {
                request.setNameCommand(parts[0]);
                request.setArgs(parts[1]);

                if (isComplexCommand(input)) {
                    if (args == null) request.setRoute(RouteGenerator.createRoute(0L));
                    else request.setRoute(new Route(args));
                } else {
