...
public class ReplaceIfGreaterCommand implements BaseCommand {
    ...
    public String execute(Request request, CollectionManager collectionManager) throws Exception {
        return Receiver.replace(request.getLogin(), request.getCommandRequest().getArgs(), request, collectionManager);
    }
    ...
    public String getDescription() {
        return "key {element} - update element by key if new bigger than element in collection with the same key";
    }
