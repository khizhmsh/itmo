...
public class DataBaseManager {
    ...
    public boolean clearUserCollection(String login) {
        int userId = getUserId(login);
        if (userId == -1) {
            return false; // Пользователь не найден
        }
        try {
            PreparedStatement statement = connection.prepareStatement(requestDML.getClearUserConnection());
            statement.setInt(1, userId);
            return statement.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Ошибка при очистке коллекции пользователя: " + e.getMessage());
            return false;
        }
    }
    ...
    // Добавление дороги
    public boolean addRoute(String login, String key, Route route) throws SQLException {
        int userId = getUserId(login);
        if (userId == -1) {
            return false; // Пользователь не найден
        }
    ...
    public boolean updateByIdRoute(String login, int id, Route route) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement(requestDML.getUpdateByIdRoute());
    ...
    public boolean updateByKeyRoute(String login, String key, Route route) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement(requestDML.getUpdateByKeyRoute());
    ...
    // Удаление дороги
    public boolean deleteRoute(String login, String routeKey) throws SQLException {
        int userId = getUserId(login);
        if (userId == -1) {
            return false; // Пользователь не найден
        }
